  ...
  #include "math.h"
  #include "stdlib.h"
  void spsp1(row,col,x0,xn,y0,yn,xp,yp,zp,
                                  dx,dy,color,xor)
  int row,col,x0,y0,xn,yn,xp,yp,zp,dx,dy,color,xor;
  { int *mx,*mn,i,j,k,l,u0,v0,u1,v1,vv,du,dv,kk;
    double x,y,z,b1,b2,b3,ca,cc,ss,sc,cca,
           ssa,u,v,xx,yy,zz;
    extern double persp1f();
    extern void sline();
    extern int POINT();
    if (dx<0) dx=-dx;
    if (dy<0) dy=-dy;
    ...
    b1=xx-((double)x0+(double)xn)/2.0;
    b2=yy-((double)y0+(double)yn)/2.0;
    b3=zz;
